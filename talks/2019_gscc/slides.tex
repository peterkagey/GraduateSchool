...
\begin{frame}{System of Recurrences: Getting one long recurrence}
  \[
    ...
    \end{bmatrix}
  \]
  % Then $\vec{a}(n + 1) = A^{n}\vec a(1)$,
  Let $\chi_A(x) = \det(xI_k - A) = x^k + \beta_{k-1}x^{k-1} + \hdots + \beta_1 x + \beta_0$ be the characteristic polynomial of $A$. %satisfies its characteristic polynomial
\end{frame}
